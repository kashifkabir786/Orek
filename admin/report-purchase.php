...
if ( isset( $_GET[ 'submit' ] ) && $_GET[ 'submit' ] == 'filter' ) {
    ...
    $query_Recordset2 = "SELECT A.`grn_id`, A.`supplier_name`, A.`supplier_inv_no`, A.`grn_date`, C.item_name, B.qnty FROM `grn` AS A INNER JOIN grn_item AS B ON A.grn_id = B.grn_id INNER JOIN item AS C ON B.item_id = C.item_id WHERE A.`grn_date` BETWEEN '{$_GET['from']}' AND '{$_GET['to']}'";
    ...
    header( "Location: reports/purchase-report.php?from=" . $_GET[ 'from' ] . "&to=" . $_GET[ 'to' ] );

} else {

    $query_Recordset2 = "SELECT A.`grn_id`, A.`supplier_name`, A.`supplier_inv_no`, A.`grn_date`, C.item_name, B.qnty FROM `grn` AS A INNER JOIN grn_item AS B ON A.grn_id = B.grn_id INNER JOIN item AS C ON B.item_id = C.item_id";
    $Recordset2 = mysqli_query( $orek, $query_Recordset2 )or die( mysqli_error( $orek ) );
    $row_Recordset2 = mysqli_fetch_assoc( $Recordset2 );
    $totalRows_Recordset2 = mysqli_num_rows( $Recordset2 );
}
...
<head>
    ...
    <title>Purchase Report - Orek</title>
    ...
    <main id="main" class="main">
        ...
        <form name="form1" action="<?php echo $editFormAction ?>" method="get" role="form">
            ...
            <div class="text-start mt-3">
                ...
                <a href="<?php echo 'reports/purchase-report.php?download=all'; ?>" class="btn btn-info">
                            ...
                            <table class="table datatable">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo $row_Recordset2['supplier_name'] ?></td>
                                        <td><?php echo $row_Recordset2['supplier_inv_no'] ?></td>
                                        <td><?php echo $row_Recordset2['grn_date'] ?></td>
