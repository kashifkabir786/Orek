...
if ( isset( $_GET[ 'category_id' ] ) ) {
  ...
  $query_Recordset2 = "SELECT * FROM category WHERE category_id='$category_id'";
...
if ( isset( $_GET[ 'success' ] ) && $_GET[ 'success' ] == "Added" ) {
  $query_Recordset2 = "SELECT * FROM category ORDER BY category_id DESC LIMIT 1";
  $Recordset2 = mysqli_query( $orek, $query_Recordset2 )or die( mysqli_error( $orek ) );
  $row_Recordset2 = mysqli_fetch_assoc( $Recordset2 );
  $totalRows_Recordset2 = mysqli_num_rows( $Recordset2 );
...
if ( ( isset( $_POST[ "MM_insert" ] ) ) && ( $_POST[ "MM_insert" ] == "form1" ) ) {
  ...
  if ( move_uploaded_file( $_FILES[ 'image_1' ][ 'tmp_name' ], $target ) ) {
    ...
    $insertSQL = sprintf( "INSERT INTO `category`(`category_name`, `refund_policy`, `exchange_policy`, `image_1`) VALUES (%s, %s, %s, %s)",
      GetSQLValueString( $_POST[ 'category_name' ], "text" ),
      GetSQLValueString( $_POST[ 'refund_policy' ], "text" ),
      GetSQLValueString( $_POST[ 'exchange_policy' ], "text" ),
      GetSQLValueString( $pic, "text" ) );
    $Result = mysqli_query( $orek, $insertSQL )or die( mysqli_error( $orek ) );

    $insertGoTo = "add-category.php?success=Added";
    if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
...
if ( ( isset( $_POST[ "MM_update" ] ) ) && ( $_POST[ "MM_update" ] == "form1" ) ) {
  $updateSQL = sprintf( "UPDATE `category` SET `category_name` = %s, `refund_policy` = %s, `exchange_policy` = %s WHERE category_id = %s",
    GetSQLValueString( $_POST[ 'category_name' ], "text" ),
    GetSQLValueString( $_POST[ 'refund_policy' ], "text" ),
    GetSQLValueString( $_POST[ 'exchange_policy' ], "text" ),
  ...
  $insertGoTo = "add-category.php?success=Updated";
  if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
    $insertGoTo .= ( strpos( $insertGoTo, '?' ) ) ? "&" : "?";
    $insertGoTo .= $_SERVER[ 'QUERY_STRING' ];
  }
...
if ( ( isset( $_POST[ "MM_photo" ] ) ) && ( $_POST[ "MM_photo" ] == "form2" ) ) {
  ...
  if ( $imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif" ) {
    ...
    if ( move_uploaded_file( $_FILES[ 'image_1' ][ 'tmp_name' ], $target ) ) {
      $insertSQL = sprintf( "UPDATE `category` SET `image_1` = %s WHERE category_id = %s",
        GetSQLValueString( $pic, "text" ),
        GetSQLValueString( $_POST[ 'category_id' ], "int" ) );
      $Result = mysqli_query( $orek, $insertSQL )or die( mysqli_error( $orek ) );

      $insertGoTo = "add-category.php?success=Image Updated";
      if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
        $insertGoTo .= ( strpos( $insertGoTo, '?' ) ) ? "&" : "?";
        $insertGoTo .= $_SERVER[ 'QUERY_STRING' ];
      }
...
<body>
    ...
    <main id="main" class="main">
        ...
        if ( isset( $_GET[ 'success' ] ) ) {
            ...
            echo '<div class="alert alert-success"> Category ' . $_GET[ 'success' ] . ' Successfully</div>';
            echo '</div>';
        }
        ?>
                <div class="col-lg-12">
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <img src="../assets/img/category/<?php echo $row_Recordset2['image_1']; ?>"
                                    ...
                                    <div class="col-md-8">
                                        <form action="<?php echo $editFormAction; ?>" method="POST"
                                            ...
                                            <div class="mb-3">
                                                <label for="image_1" class="form-label">Select New Category
                                                    Image</label><br>
                                            ...
                                            <div class="text-start">
                                                ...
                                                <input type="hidden" name="category_id"
                                                    value="<?php echo $row_Recordset2['category_id']; ?>" />
                        ...
                        <form method="POST" class="row g-3" name="form1" role="form"
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" class="form-control" name="category_name"
                                    value="<?php if($flag) echo $row_Recordset2['category_name'] ?>" />
                            </div>
                            <div class="col-12">
                            ...
                            <div class="col-12">
                                <label class="control-label">Exchange Policy:</label>
                                <select name="exchange_policy" class="form-control">
                                    <option value="">Select Exchange Policy</option>
                                    <option value="7 Days Exchange"
                                        <?php if($flag && $row_Recordset2['exchange_policy'] == '7 Days Exchange') echo "selected" ?>>
                                        7 Days Exchange </option>
                                    <option value="Non Exchangeble"
                                        <?php if($flag && $row_Recordset2['exchange_policy'] == 'Non Exchangeble') echo "selected" ?>>
                                        Non Exchangeble </option>
                                </select>
                            </div>
                            <?php if(!$flag) { ?>
                            <div class="col-12">
