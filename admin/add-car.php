...
if ( isset( $_GET[ 'car_id' ] ) ) {
  ...
  $query_Recordset2 = "SELECT * FROM car WHERE car_id='$car_id'";
...
if ( isset( $_GET[ 'success' ] ) && $_GET[ 'success' ] == "Added" ) {
  $query_Recordset2 = "SELECT * FROM car ORDER BY car_id DESC LIMIT 1";
  $Recordset2 = mysqli_query( $car_spare, $query_Recordset2 )or die( mysqli_error( $car_spare ) );
  $row_Recordset2 = mysqli_fetch_assoc( $Recordset2 );
  $totalRows_Recordset2 = mysqli_num_rows( $Recordset2 );

  $car_id = $row_Recordset2[ 'car_id' ];
  $flag = true;
}
...
if ( ( isset( $_POST[ "MM_insert" ] ) ) && ( $_POST[ "MM_insert" ] == "form1" ) ) {
  ...
  if ( move_uploaded_file( $_FILES[ 'image_1' ][ 'tmp_name' ], $target ) ) {
    ...
    $insertSQL = sprintf( "INSERT INTO `car`(`car_name`, `image_1`) VALUES (%s, %s)",
      GetSQLValueString( $_POST[ 'car_name' ], "text" ),
      GetSQLValueString( $pic, "text" ) );
    $Result = mysqli_query( $car_spare, $insertSQL )or die( mysqli_error( $car_spare ) );

    $insertGoTo = "add-car.php?success=Added";
    if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
...
if ( ( isset( $_POST[ "MM_photo" ] ) ) && ( $_POST[ "MM_photo" ] == "form2" ) ) {
  ...
  if ( $imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif" ) {
    ...
    if ( move_uploaded_file( $_FILES[ 'image_1' ][ 'tmp_name' ], $target ) ) {
      $insertSQL = sprintf( "UPDATE `car` SET `image_1` = %s WHERE car_id = %s",
        GetSQLValueString( $pic, "text" ),
        GetSQLValueString( $_POST[ 'car_id' ], "int" ) );
      $Result = mysqli_query( $car_spare, $insertSQL )or die( mysqli_error( $car_spare ) );

      $insertGoTo = "add-car.php?success=Image Updated";
      if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
        $insertGoTo .= ( strpos( $insertGoTo, '?' ) ) ? "&" : "?";
        $insertGoTo .= $_SERVER[ 'QUERY_STRING' ];
      }
...
<div class="container-fluid">
  <div class="row">
    <div class="col-md-offset-2 col-md-10">
      <h3><strong>ADD car</strong></h3>
      <hr/>
      <?php
      if ( isset( $_GET[ 'success' ] ) ) {
        echo '<div class="col-md-12">';
        echo '<div class="alert alert-success">car ' . $_GET[ 'success' ] . ' Successfully</div>';
        echo '</div>';
      }
      ?>
      <div class="margin-50"> <a href="car.php" class="btn btn-primary">Back</a></div>
      <div class="row margin-70">
        <div class="col-md-10">
          <?php
          ...
          <h4>Image 1</h4>
          <div class="row">
            <div class="col-md-4"> <img src="../assets/img/car/<?php echo $row_Recordset2['image_1'] ?>" width="60%" class="img-thumbnail"> </div>
            <div class="col-md-4">
              <form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="form2" role="form">
                <div class="form-group">
                  <label class="control-label" for="image_1">Select car Image</label>
                  <input type="file" name="image_1" id="image_1" />
                </div>
                <div class="form-group">
                  <button type="submit" id="upload" class="btn btn-info">Upload</button>
                  <input type="hidden" name="car_id" value="<?php echo $row_Recordset2['car_id'] ?>"/>
          ...
          <form method="POST" name="form1" id="form1" role="form" action="<?php echo $editFormAction; ?>" enctype="multipart/form-data">
            <div class="form-group">
              ...
              <input type="text" name="car_name" class="form-control" value="<?php if($flag) echo $row_Recordset2['car_name']; ?>" required>
            </div>
            <?php if(!$flag) { ?>
            <div class="form-group">
              <label class="control-label">Add car Logo</label>
