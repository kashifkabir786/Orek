...
if ( isset( $_GET[ 'category_id' ] ) ) {
    ...
    $photoSQL = sprintf( "SELECT `image_1` FROM `category` WHERE `category_id` = %s", GetSQLValueString( $_GET[ 'category_id' ], "int" ) );
    $photo = mysqli_query( $orek, $photoSQL )or die( mysqli_error( $orek ) );
    $row_photo = mysqli_fetch_assoc( $photo );

    $deleteSQL = sprintf( "DELETE FROM `category` WHERE category_id = %s",
        GetSQLValueString( $_GET[ 'category_id' ], "int" ) );
    $Result1 = mysqli_query( $orek, $deleteSQL )or die( mysqli_error( $orek ) );
	
	unlink("../assets/img/category/" . $row_photo['image_1']);
