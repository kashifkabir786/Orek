...
<?php
$flag = false;
if ( isset( $_GET[ 'category_level2_id' ] ) ) {
  $category_level2_id = $_GET[ 'category_level2_id' ];

  $query_Recordset2 = "SELECT * FROM category_level2 WHERE category_level2_id='$category_level2_id'";
  ...
  $flag = true;
    ...
    $query_Recordset3 = "SELECT * FROM category_level1";
    $Recordset3 = mysqli_query( $orek, $query_Recordset3 )or die( mysqli_error( $orek ) );
    $row_Recordset3 = mysqli_fetch_assoc( $Recordset3 );
    $totalRows_Recordset3 = mysqli_num_rows( $Recordset3 );

if ( isset( $_GET[ 'success' ] ) && $_GET[ 'success' ] == "Added" ) {
  $query_Recordset2 = "SELECT * FROM category_level2 ORDER BY category_level2_id DESC LIMIT 1";
  $Recordset2 = mysqli_query( $orek, $query_Recordset2 )or die( mysqli_error( $orek ) );
  $row_Recordset2 = mysqli_fetch_assoc( $Recordset2 );
  $totalRows_Recordset2 = mysqli_num_rows( $Recordset2 );

  $category_level2_id = $row_Recordset2[ 'category_level2_id' ];
  $flag = true;
}
...
if ( ( isset( $_POST[ "MM_insert" ] ) ) && ( $_POST[ "MM_insert" ] == "form1" ) ) {
  ...
  if ( move_uploaded_file( $_FILES[ 'image' ][ 'tmp_name' ], $target ) ) {
    ...
    $insertSQL = sprintf( "INSERT INTO `category_level2`(`category_level1_id`, `category_level2_name`, `refund_policy`, `image`) VALUES (%s, %s, %s, %s)",
      GetSQLValueString( $_POST[ 'category_level1_id' ], "text" ),
      GetSQLValueString( $_POST[ 'category_level2_name' ], "text" ),
    ...
    $insertGoTo = "add-category_level2.php?success=Added";
    if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
...
if ( ( isset( $_POST[ "MM_update" ] ) ) && ( $_POST[ "MM_update" ] == "form1" ) ) {
  $updateSQL = sprintf( "UPDATE `category_level2` SET `category_level1_id` = %s, `category_level2_name` = %s, `refund_policy` = %s WHERE category_level2_id = %s",
    GetSQLValueString( $_POST[ 'category_level1_id' ], "text" ),
    GetSQLValueString( $_POST[ 'category_level2_name' ], "text" ),
    GetSQLValueString( $_POST[ 'refund_policy' ], "text" ),
    GetSQLValueString( $_POST[ 'category_level2_id' ], "int" ) );
  $Result = mysqli_query( $orek, $updateSQL )or die( mysqli_error( $orek ) );

  $insertGoTo = "add-category_level2.php?success=Updated";
  if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
    $insertGoTo .= ( strpos( $insertGoTo, '?' ) ) ? "&" : "?";
    $insertGoTo .= $_SERVER[ 'QUERY_STRING' ];
  }
...
if ( ( isset( $_POST[ "MM_photo" ] ) ) && ( $_POST[ "MM_photo" ] == "form2" ) ) {
  ...
  if ( $imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif" ) {
    ...
    if ( move_uploaded_file( $_FILES[ 'image' ][ 'tmp_name' ], $target ) ) {
      $insertSQL = sprintf( "UPDATE `category_level2` SET `image` = %s WHERE category_level2_id = %s",
        GetSQLValueString( $pic, "text" ),
        GetSQLValueString( $_POST[ 'category_level2_id' ], "int" ) );
      $Result = mysqli_query( $orek, $insertSQL )or die( mysqli_error( $orek ) );

      $insertGoTo = "add-category_level2.php?success=Image Updated";
      if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
        $insertGoTo .= ( strpos( $insertGoTo, '?' ) ) ? "&" : "?";
        $insertGoTo .= $_SERVER[ 'QUERY_STRING' ];
      }
...
<head>
    ...
    <title>Add Category Lavel2 - Orek</title>
...
<body>
    ...
    <main id="main" class="main">
        <div class="pagetitle">
            <h1>Add Category Level2</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="dashboard.php">Home</a></li>
                    <li class="breadcrumb-item active">Add Category Level2</li>
                </ol>
            </nav>
        </div>
        <!-- End Page Title -->
        ...
        <section class="section dashboard">
            <div class="row">
                <div class="col-md-4 mb-3">
                    <a href="category_level2.php" class="btn btn-secondary">Back</a>
                </div>
                <!-- Left side columns -->
                <?php
        if ( isset( $_GET[ 'success' ] ) ) {
            echo '<div class="col-md-12">';
            echo '<div class="alert alert-success"> Category Level2 ' . $_GET[ 'success' ] . ' Successfully</div>';
            echo '</div>';
        }
        ?>
                <div class="col-lg-12">
                    <div class="row">
                                    ...
                                    <div class="col-md-8">
                                        <form action="<?php echo $editFormAction; ?>" method="POST"
                                            ...
                                            <div class="text-start">
                                                ...
                                                <input type="hidden" name="category_level2_id"
                                                    value="<?php echo $row_Recordset2['category_level2_id']; ?>" />
                        ...
                        <form method="POST" class="row g-3" name="form1" role="form"
                            ...
                            <div class="col-12">
                                ...
                                <select name="category_level1_id" required class="form-control">
                                    <option value="">Select Category Level1 </option>
                                    <?php do{ ?>
                                    <option value="<?php echo $row_Recordset3['category_level1_id'] ?>"
                                        <?php if($flag && $row_Recordset2['category_level1_id'] == $row_Recordset3['category_level1_id']) echo "selected"; ?>>
                                        <?php echo $row_Recordset3['category_level1_name']; ?></option>
                                    <?php }while($row_Recordset3=mysqli_fetch_assoc($Recordset3)) ?>
                                </select>
                            </div>
                            <div class="col-12">
                                <label for="category_level2_name" class="form-label">Category Level2 Name</label>
                                <input type="text" class="form-control" name="category_level2_name"
                                    value="<?php if($flag) echo $row_Recordset2['category_level2_name'] ?>" />
                            </div>
                            <div class="col-12">
                            ...
                            <div class="text-center">
                                ...
                                <input type="hidden" name="category_level2_id"
                                    value="<?php echo $category_level2_id; ?>">
                                <?php } else { ?>
